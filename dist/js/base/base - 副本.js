	            ...
	            $(personal).each(function(index,item){
	                switch (item.type) {
	                    case 'dropdown':
	                        ...
	                        template += '<div class="weui_cells"><div class="weui_cell weui_cell_select weui_select_after"><div class="weui_cell_hd"><label for="" class="weui_label">'+ item.label +'</label></div><div class="weui_cell_bd weui_cell_primary"><select class="weui_select" data-label="'+ item.label +'" data-type="dropdown" name="'+ item.name +'">'+ options +'</select></div></div></div>'
	                        break;
	                    case 'radio':  //单选框
	                        //console.log(item.options)
	                        var check ="";
	                        $(item.options).each(function(i,val){
								var isChecked = val.checked ? 'checked="true"' : '';
	                            check += '<label class="weui_cell weui_check_label" for="'+ (item.name + i) +'"><div class="weui_cell_bd weui_cell_primary"><p>'+ val.label +'</p></div><div class="weui_cell_ft"><input type="radio" '+ isChecked +' class="weui_check" data-type="radio" data-required="'+ item.required +'" name="'+ item.name +'"  value="'+ val.label +'" id="'+ (item.name + i) +'"><span class="weui_icon_checked"></span></div></label>'
	                        })
	                        template += '<div class="weui_cells"><div class="weui_cells_title weui_cells_tit">'+ item.label +'</div><div class="weui_cells weui_cells_checkbox">'+ check +'</div></div>'
	                        break;
	                    case 'checkbox':
	                        var checkbox = "";
	                        $(item.options).each(function(i,val){
								var isChecked = val.check ? 'checked="checked"' : '';
	                            checkbox += '<label class="weui_cell weui_check_label" for="'+ (item.name + i) +'"><div class="weui_cell_bd weui_cell_primary"><p>'+ val.label +'</p></div><div class="weui_cell_hd checkbox_icon"><input type="checkbox" '+ isChecked +' class="weui_check" data-type="checkbox" name="'+ item.name +'" id="'+ (item.name + i) +'" value="'+ val.label +'"><i class="weui_icon_checked"></i></div></label>'
	                        })
	                        template += '<div class="weui_cells"><div class="weui_cells_title weui_cells_tit">'+ item.label +'</div><div class="weui_cells weui_cells_radio">'+ checkbox +'</div></div>'
	                        break;
	                    case 'image_file':
	                        template += '<div class="weui_cells"><div class="weui_cell"><div class="weui_cells_tips">'+ item.label +'</div><div class="weui_cell_bd weui_cell_primary"><div class="weui_uploader_input_wrp"><input class="weui_uploader_input" data-required="'+ item.required +'" data-type="image_file" data-label="'+ item.label +'" name="'+ item.name +'" type="file" accept="image/jpg,image/jpeg,image/png,image/gif" multiple=""></div></div></div></div>'
	                        break;
	                    case 'label':
	                    ...
	                    case 'certificate': //证件
	                        ...
	                        template += '<div class="weui_cells"><div class="weui_cell weui_cell_select weui_select_before"><div class="weui_cell_hd"><select data-type="certificate" class="weui_select" data-required="'+ item.required +'" data-alias="'+ item.name +'" data-label="'+ item.label +'" name="'+ item.subname[0] +'">'+ options +'</select></div><div class="weui_cell_bd weui_cell_primary"><input class="weui_input" data-type="certificate" data-alias="'+ item.name +'" data-required="'+ item.required +'" name="'+ item.subname[1] +'" type="text" placeholder="请输入证件号码"></div></div></div>'
	                        break;
						case 'number':
							template += '<div class="weui_cells"><div class="weui_cell"><div class="weui_cell_hd"><label class="weui_label">'+ item.label +'</label></div><div class="weui_cell_bd weui_cell_primary"><input data-required="'+ item.required +'" data-type="number" data-label="'+ item.label +'" name="'+ item.name +'" type="number" pattern="[0-9]*" placeholder="请输入'+ item.label +'" class="weui_input"></div></div></div>';
							break;
						case 'tel':
								template += '<div class="weui_cells"><div class="weui_cell"><div class="weui_cell_hd"><label class="weui_label">'+ item.label +'</label></div><div class="weui_cell_bd weui_cell_primary"><input data-required="'+ item.required +'" data-type="tel" data-label="'+ item.label +'" name="'+ item.name +'" type="tel" pattern="[0-9]*" placeholder="请输入'+ item.label +'" class="weui_input"></div></div></div>';
								break;
	                    default:
	                        template += '<div class="weui_cells"><div class="weui_cell"><div class="weui_cell_hd"><label class="weui_label">'+ item.label +'</label></div><div class="weui_cell_bd weui_cell_primary"><input data-required="'+ item.required +'" data-type="texts" data-label="'+ item.label +'" name="'+ item.name +'" type="text" placeholder="请输入'+ item.label +'" class="weui_input"></div></div></div>';
	                }
	            });
	            template += '</form>';
	        };
